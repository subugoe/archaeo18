(function($) {
	...
	$.fn.draw = function(o,obj,selectedpage){
		...
		obj.data("left",_divwrapleft);
		
		var _ulwrapdiv	= $(document.createElement('div')).css('overflow','hidden');
		obj.data("trigger",_ulwrapdiv);		
		...
		outsidewidth = outsidewidth_tmp - _first.parent().width() -3;
		obj.data("outsideWidth",outsidewidth);
		if(ver == 'ie7'){
		...
		_ulwrapdiv.find('li').click(function(e){
			...
			if(ver == 'ie7')
				...
				_ulwrapdiv.animate({scrollLeft: left + tmp - _first.parent().width() + selobj.width()/2 + 'px'});	
			o.onChange(currval);	
		});
	...
	$.fn.applyWidth = function(width){
		var div = pagination.data('trigger');
		var left = pagination.data('left');
		var right = pagination.data('right');
		div.css( 'width', (width-$(left).width()-$(right).width()-10)+'px' );
		right.css( 'left', ($(div).width()+$(left).width()+10)+'px' );
		outsidewidth = width-$(left).width()-$(right).width()-10;
	}
})(jQuery);
